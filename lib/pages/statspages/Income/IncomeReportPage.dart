...
class IncomeReportPage extends StatefulWidget {
  ...
  const IncomeReportPage({super.key, required this.transactions});
  @override
...
class _IncomeReportPageState extends State<IncomeReportPage>
  ...
  Map<String, double> groupedIncome = {};
  ...
  void _calculateIncome() {
    ...
    if (currentView == "Year") {
      ...
      for (var txn in widget.transactions) {
        ...
        groupedIncome[key] = (groupedIncome[key] ?? 0) + amt;
      }
      totalIncome = groupedIncome.values.fold(0.0, (a, b) => a + b);
      groupedIncome = Map.fromEntries(
        groupedIncome.entries.toList()..sort((a, b) {
      ...
      // Only weekly breakdown (no month total bar)
      for (var w in weeks) {
        final label = w['label'];
        final weekAmt = weekTotals[w['index']] ?? 0;
        groupedIncome[label] = weekAmt;
      }
      totalIncome = weekTotals.values.fold(0.0, (a, b) => a + b);
      ...
      selectedWeek = weeks[selWeekIdx - 1];
      ...
      totalIncome = tempMap.values.fold(0.0, (a, b) => a + b);
      final entries = tempMap.entries.toList()
        ..sort((a, b) => keyToDate[a.key]!.compareTo(keyToDate[b.key]!));
      groupedIncome = Map.fromEntries(entries);
    }
  }
  ...
  String _chartTitleForCurrentView() {
    if (currentView == "Year") {
      ...
      return 'Monthly income for $y';
    } else if (currentView == "Month") {
      final y = filterYear ?? DateTime.now().year;
      final m = filterMonth ?? DateTime.now().month;
      return 'Weekly income for ${DateFormat('MMM yyyy').format(DateTime(y, m))}';
    } else {
      final y = filterYear ?? DateTime.now().year;
      final m = filterMonth ?? DateTime.now().month;
      final w = filterWeek ?? weekNumber(DateTime.now());
      return 'Daily income for Week $w (${DateFormat('MMM yyyy').format(DateTime(y, m))})';
    }
  }
  ...
  Widget build(BuildContext context) {
    final List<_ChartData> chartData = groupedIncome.entries
              ...
              child: Column(
                ...
                children: [
                  ...
                  groupedIncome.isEmpty
                      ...
                      : Column(
                          children: groupedIncome.entries.map((e) {
                            return Card(
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(12),
                              ),
