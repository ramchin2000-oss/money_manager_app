...
//   const TransferReportPage({Key? key, required this.transfers})
//     : super(key: key);
//
//   @override
...
//   Map<String, double> groupedTransfer = {};
...
//     final lastDay = DateTime(year, month + 1, 0); // inclusive end
//     for (int i = 0; i < 4; i++) {
...
//   }
//
//   void _calculateTransfer() {
//     groupedTransfer.clear();
//     totalTransfer = 0;
//     final now = DateTime.now();
//
//     if (currentView == "Year") {
//       final int yearToShow = filterYear ?? now.year;
//       for (var txn in widget.transfers) {
//         if ((txn['type'] ?? "Transfer") != 'Transfer') continue;
...
//         groupedTransfer[key] = (groupedTransfer[key] ?? 0) + amt;
//       }
//       totalTransfer = groupedTransfer.values.fold(0.0, (a, b) => a + b);
//       groupedTransfer = Map.fromEntries(
//         groupedTransfer.entries.toList()..sort((a, b) {
...
//         groupedTransfer[label] = weekAmt;
//       }
//       totalTransfer = weekTotals.values.fold(0.0, (a, b) => a + b);
...
//         ..sort((a, b) => keyToDate[b.key]!.compareTo(keyToDate[a.key]!));
//       groupedTransfer = Map.fromEntries(entries);
//       totalTransfer = tempMap.values.fold(0.0, (a, b) => a + b);
//     }
//   }
//
...
//       return 'Monthly transfer for $y';
//     } else if (currentView == "Month") {
//       final y = filterYear ?? DateTime.now().year;
//       final m = filterMonth ?? DateTime.now().month;
//       return 'Weekly transfer for ${DateFormat('MMM yyyy').format(DateTime(y, m))}';
//     } else {
//       final y = filterYear ?? DateTime.now().year;
//       final m = filterMonth ?? DateTime.now().month;
//       final w = filterWeek ?? weekNumber(DateTime.now());
//       return 'Daily transfer for Week $w (${DateFormat('MMM yyyy').format(DateTime(y, m))})';
//     }
//   }
//
...
//     final List<_ChartData> chartData = groupedTransfer.entries
...
//                         tween: Tween<double>(begin: 0, end: totalTransfer),
...
//                         )
//                       : Column(
//                           children: groupedTransfer.entries.map((e) {
//                             return Card(
//                               shape: RoundedRectangleBorder(
//                                 borderRadius: BorderRadius.circular(12),
//                               ),
